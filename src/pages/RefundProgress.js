...
const RefundProgress = () => {
  ...
  const fetchRefundStatus = async () => {
    try {
      ...
      setRefundStatus(response.data); // Set refund status dynamically from API
    } catch (error) {
      ...
      <div style={{ marginTop: '30px' }}>
        ...
        <div
          style={{
            ...
            borderRadius: '5px',
            overflow: 'hidden',
            marginBottom: '10px', // Spacing between bar and stage names
          }}
        >
          <div
            style={{
        ...
        <div
          style={{
            ...
            marginTop: '10px', // Spacing between progress bar and names
          }}
        >
            ...
            <div key={index} style={{ textAlign: 'center', width: '20%' }}>
              <div
                style={{
                  ...
                  height: '40px',
                  borderRadius: '50%',
                  backgroundColor: index < currentIndex ? '#4CAF50' : index === currentIndex ? '#4CAF50' : '#ddd',                  display: 'flex',
                  justifyContent: 'center',
                  alignItems: 'center',
                  margin: '0 auto',
                  fontSize: '14px',
                  color: index < currentIndex ? '#fff' : index === currentIndex ? '#fff' : '#000',
                }}
              >
                {index + 1}
              </div>
...
//   const { itemId } = useParams();
//   const [item, setItem] = useState(null);
//   const [refundStatus, setRefundStatus] = useState(''); // Fetch refund status dynamically
// const [isLoading, setIsLoading] = useState(true);
//   const BASE_URL = process.env.REACT_APP_API_BASE_URL;
